...
int nccl_net_ofi_rdma_domain_t::alloc_and_reg_flush_buff(int dev_id, nccl_net_ofi_rdma_ep_t *ep)
{
	...
	ret = this->reg_internal_mr(this->flush_buff.buffer, system_page_size,
				NCCL_PTR_HOST, NCCL_OFI_RDMA_CONTROL_RAIL, ep, &mr_handle);
	if (OFI_UNLIKELY(ret != 0)) {
	...
	if (nccl_ofi_dmabuf_viable_and_supported(nic_prov)) {
		...
		*/
		...
		ret = this->reg_internal_mr_dma_buf(this->flush_buff.buffer, fd, offset, system_page_size,
						NCCL_PTR_CUDA, NCCL_OFI_RDMA_CONTROL_RAIL, ep, &mr_handle);
		close(fd);
	} else {
		ret = this->reg_internal_mr(this->flush_buff.buffer, system_page_size, NCCL_PTR_CUDA, NCCL_OFI_RDMA_CONTROL_RAIL, ep, &mr_handle);
	}
...
static int post_eager_copy(nccl_net_ofi_rdma_req_t *req)
{
	...
	uint64_t buff_offset = (uint64_t)rx_buff - rx_mr_handle->base_addr;
...
static int post_flush_req(nccl_net_ofi_rdma_req_t *req)
{
	...
	for (uint16_t rail_id = 0; rail_id < ep->num_rails; rail_id++) {
		...
		uint64_t buff_offset = (uint64_t)flush_data->data - flush_data->mr_handle->base_addr;
...
static int post_flush_req(nccl_net_ofi_rdma_req_t *req)
{
	...
	for (uint16_t rail_id = 0; rail_id < ep->num_rails; rail_id++) {
		...
		uint64_t buff_offset = (uint64_t)domain->flush_buff.buffer - domain->flush_buff.mr_handle->base_addr;
		rc = fi_read(comm_rail->local_ep,
			(void *)host_buff_addr,
			NCCL_OFI_DEFAULT_CPU_CACHE_LINE_SIZE, desc, comm_rail->local_addr,
			buff_offset,
								...
								nccl_ofi_rdma_connection_info_t *conn_msg)
{
	...
	conn_msg->ctrl_addr = (uint64_t)ctrl_msg - ctrl_msg_mr_handle->base_addr;
...
int nccl_net_ofi_rdma_ep_t::create_send_comm(nccl_net_ofi_rdma_send_comm_t **s_comm)
{
	...
	*s_comm = NULL;
	...
	ret = domain_ptr->reg_internal_mr(ret_s_comm->ctrl_mailbox, sizeof(nccl_net_ofi_ctrl_msg_t) * NCCL_OFI_CTRL_MAILBOX_SIZE,
						  NCCL_PTR_HOST, NCCL_OFI_RDMA_CONTROL_RAIL, this, &ret_s_comm->ctrl_mr_handle);
	if (ret != 0) {
